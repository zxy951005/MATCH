...
def initialize_from_env(experiment, logdir=None):
    ...
    config["log_dir"] = mkdirs(os.path.join(config["log_root"], "CRAFT-nok"))
...
class GraphAttention(object):
  ...
  def call(self, inputs):
    ...
    A = self.inputs[1]  # Adjacency matrix (2*4, 2*4)
    D = self.inputs[2]  # Adjacency matrix (2*4, 2*4)
    ...
    for head in range(self.attn_heads):
      with tf.variable_scope("head_{}".format(head)):       
        ...
        #dense=tf.multiply(attn_for_self,D)+tf.multiply(attn_for_neighs,D)
        #node_features = tf.matmul(D, features)   # (numsent*maxseent x 8)
