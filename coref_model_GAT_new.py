...
class CorefModel(object):
    def __init__(self, config):
        ...
        #adj
        input_props.append((tf.float32, [None, None, None, None, 1 ]))  
        #dep
        input_props.append((tf.float32, [None, None, None, None, self.dep_emb_size]))
    ...
    def tensorize_example(self, example, is_training):
        ...
        for i in range(len(dep_local)):
          depnew_s=dep_local[i]+dep_global[i]
          
          depall = []
          for l1 in depnew_s:
            if (l1 not in depall) and (l1[0] not in ["root","ROOT"]):
              depall.append(l1)
          ...
          for i2 in reversed(deleteindex):
            del depall[i2]
          dep_doc.append(depall)
        
        if doc_key=="PMID-9971788":  
          dep_doc[1]=dep_doc[1][:-1]
        ...
        for i in range(len(dep_doc)):
            #print(doc_key,2*len(dep_doc[i])+sent_len[i],np.count_nonzero(adj_emb[i]))
            assert 2*len(dep_doc[i])+sent_len[i]==np.count_nonzero(adj_emb[i])
        ...
        example_tensors = (tokens, context_word_emb, head_word_emb, lm_emb, bert_emb, char_index, text_len, is_training, gold_starts, gold_ends, cluster_ids, number_ids, MM_ids, context_OMCS_emb, context_organisms_emb, context_proteins_emb, context_mygenes_emb, context_genes_emb, context_interaction_types_emb,dep_emb,adj_emb)
        ...
        #adj   
        A=np.zeros([num_sentences,num_sentences,max_sentence_length,max_sentence_length,1])                
        for i in range(len(sentences)):
            A[i][i]=adj_emb[i]
           
        #dep    
        AD=np.zeros([num_sentences,num_sentences,max_sentence_length,max_sentence_length,self.dep_emb_size])  
        for i in range(len(sentences)):
            AD[i][i]=dep_emb[i]

            
        for i in range(len(A)):
            #print(np.count_nonzero(A[i][i]),2*len(dep_doc[i])+sent_len[i])
            assert np.count_nonzero(A[i][i])==2*len(dep_doc[i])+sent_len[i]
                      
        
        tup2=(A,AD)
        new_example_tensors=example_tensors+tup2
           
        return new_example_tensors

    def truncate_example(self, tokens, context_word_emb, head_word_emb, lm_emb, bert_emb, char_index, text_len, is_training, gold_starts, gold_ends, cluster_ids, number_ids, MM_ids, OMCS_emb, organisms_emb, proteins_emb, mygenes_emb, genes_emb, interaction_types_emb,dep_emb,adj_emb):
        ...
        return tokens, context_word_emb, head_word_emb, lm_emb, bert_emb, char_index, text_len, is_training, gold_starts, gold_ends, cluster_ids, number_ids, MM_ids, OMCS_emb, organisms_emb, proteins_emb, mygenes_emb, genes_emb, interaction_types_emb,dep_emb,adj_emb
    ...
    def get_predictions_and_loss(self, tokens, context_word_emb, head_word_emb, lm_emb, bert_emb, char_index, text_len, is_training, gold_starts, gold_ends, cluster_ids, number_ids, MM_ids, OMCS_emb, organisms_emb, proteins_emb, mygenes_emb, genes_emb, interaction_types_emb,dep_emb,adj_emb,sent_adj_emb,sent_dep_emb):
        ...
        gatoutput_all=self.GAT_encode(A_tmp,dep_emb_reshape, KB_emb_tmp, text_len_mask,self.gat_dropout,self.n_attn_heads,self.GAToutsize)
